...
// the staging pad: a 42 x 20 grid draped over the sand under the show
...
// opening formation: a great glowing wall, 42 x 20, facing camp
function genWall(N, rand) {
  const f = alloc(N);
  const COLS = 42;
  for (let i = 0; i < N; i++) {
    const c = i % COLS, r = (i / COLS) | 0;
    f.pts[i * 3] = (c - (COLS - 1) / 2) * 1.5;
    f.pts[i * 3 + 1] = (r - 9.5) * 1.4;
    f.pts[i * 3 + 2] = (rand() - 0.5) * 0.9;
    f.u[i] = r / 19;              // 0 bottom -> 1 top
    f.v[i] = c / (COLS - 1);
  }
  return f;
}

function genSphere(N, rand, R = 23) {
  const f = alloc(N);
...
function genHelix(N, rand) {
  const f = alloc(N);
  const R = 8.5, H = 46, TURNS = 2.6;
  const STRAND = 320;             // per strand; the rest are rungs
  let i = 0;
  for (let s = 0; s < 2; s++) {
...
function genCactus(N, rand) {
  ...
  const pieces = [
    { a: [0, -26, 0], b: [0, 26, 0], R: 4.5, n: 400, tipY: 22 },
    { a: [-4, -9, 0], b: [-13, -9, 0], R: 3.0, n: 80, tipY: null },
    { a: [-13, -9, 0], b: [-13, 11, 0], R: 3.0, n: 150, tipY: 8 },
    { a: [4, -1, 0], b: [11.5, -1, 0], R: 3.0, n: 80, tipY: null },
    { a: [11.5, -1, 0], b: [11.5, 15, 0], R: 3.0, n: 130, tipY: 11 },
  ];
  ...
  for (; i < N; i++) {
    f.pts[i * 3] = (rand() - 0.5) * 8;
    f.pts[i * 3 + 1] = (rand() - 0.5) * 48;
    f.pts[i * 3 + 2] = (rand() - 0.5) * 8;
    f.v[i] = (f.pts[i * 3 + 1] + 26) / 52;
  }
  return f;
}
...
function genHeart(N, rand) {
  ...
  const heart = (t, out) => {
    const s = Math.sin(t);
    out.x = 16 * s * s * s * K;
    out.y = (13 * Math.cos(t) - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t)) * K + 3;
    return out;
  };
  ...
  for (let i = 0; i < N; i++) {
    const onRim = i < RIM;
    heart(rand() * TWO_PI, p);
    // fill points shrink toward the visual center (0, -2)
    const s = onRim ? 1 : Math.sqrt(rand()) * 0.88;
    f.pts[i * 3] = p.x * s + (rand() - 0.5) * 0.8;
    f.pts[i * 3 + 1] = -2 + (p.y + 2) * s + (rand() - 0.5) * 0.8;
    f.pts[i * 3 + 2] = (rand() - 0.5) * 3.2;
    f.u[i] = onRim ? 1 : s;       // rim-ness for coloring
  }
  return f;
}

// homage to the host: a frozen firework — drooping rays around a hot core
function genBurst(N, rand) {
  const f = alloc(N);
  const RAYS = 20, PER = 36;
  ...
  for (let rIx = 0; rIx < RAYS; rIx++) {
    ...
    for (let k = 0; k < PER; k++, i++) {
      const q = (k + 0.5) / PER;
      const r = 2.5 + 24.5 * Math.pow(q, 0.9);
      const jit = 0.4 + q * 0.7;
      f.pts[i * 3] = dx * r + (rand() - 0.5) * jit;
      f.pts[i * 3 + 1] = dy * r - 6 * q * q + (rand() - 0.5) * jit; // willow droop
...
function genMoonStar(N, rand) {
  ...
  for (let i = 0; i < MOON; i++) {
    ...
    for (let tries = 0; tries < 40; tries++) {
      const r = 16 * Math.sqrt(rand());
      const th = rand() * TWO_PI;
      x = Math.cos(th) * r;
      y = Math.sin(th) * r;
      const cx = x - 6.2, cy = y;
      if (cx * cx + cy * cy > 13.8 * 13.8) break;
    }
    f.pts[i * 3] = x;
    f.pts[i * 3 + 1] = y;
    f.pts[i * 3 + 2] = (rand() - 0.5) * 2.6;
    f.u[i] = 0;
  }
  // 5-point star at the crescent's opening
  const SCX = 11.5, SCY = 6.5, RO = 5.4, RI = 2.3;
  const vx = [], vy = [];
  for (let k = 0; k < 10; k++) {
  ...
  for (let i = MOON; i < N; i++) {
    ...
    const s = i < MOON + 150 ? 1 : Math.sqrt(rand()); // outline + light fill
    f.pts[i * 3] = SCX + (x0 + (x1 - x0) * fr) * s;
    f.pts[i * 3 + 1] = SCY + (y0 + (y1 - y0) * fr) * s;
    f.pts[i * 3 + 2] = (rand() - 0.5) * 2.0;
    f.u[i] = 1;
  }
  return f;
}
...
function genText(N, rand, text = 'GOOD NIGHT') {
  ...
  for (let i = 0; i < N; i++) {
    ...
    f.pts[i * 3 + 1] = (H / 2 - y) * SCALE + 3;
    f.pts[i * 3 + 2] = (rand() - 0.5) * 2.2;
...
export class DroneShow {
  constructor(scene, audio, groundHeight) {
    ...
    const cx = -110, cz = -35;
    this.center = new THREE.Vector3(cx, groundHeight(cx, cz) + 54, cz);
    ...
    // movers leave first so everyone lands the shape together)
    const beatScale = (t) => 1 + 0.05 * heartbeat(t);
    const burstScale = (t) => 1 + 0.05 * Math.sin(t * 0.5);
    this.scenes = [
      {
      ...
      {
        label: 'DNA HELIX', dur: 22, spin: 0.55, stagger: 2.6, mode: 'sync',
        gen: genHelix,
        color: (i, u, v, t, out) => {
      ...
      {
        label: 'STARBURST', dur: 22, spin: 0.09, stagger: 2.6, mode: 'sync',
        gen: genBurst, scale: burstScale,
        color: (i, u, v, t, out) => {
          out.copy(GOLD).lerp(SCARLET, u).lerp(PINK, Math.max(0, u - 0.82) * 5);
          // detonation wave rolling out along the rays, forever
          const w = (t * 0.3) % 1;
          const d = (u - w) / 0.13;
          out.multiplyScalar(0.75 + 1.5 * Math.exp(-d * d));
        },
      },
      {
  ...
  // world position of formation slot j at spin angle (c,s) and scale k
  _slotWorld(data, j, c, s, k, out) {
    if (data.world) return out.fromArray(data.pts, j * 3);
    const lx0 = data.pts[j * 3], ly = data.pts[j * 3 + 1], lz0 = data.pts[j * 3 + 2];
    const lx = (lx0 * c + lz0 * s) * k;
    const lz = (-lx0 * s + lz0 * c) * k;
    const { center, right, front } = this;
    return out.set(
  ...
  _enter(k) {
    ...
    if (s.mode === 'sync') {
      for (let r = 0; r < N; r++) {
        const i = droneRank[r];
        this._slotWorld(s.data, orderB[r], 1, 0, 1, _v1);
  ...
  update(dt, time) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (!held) {
        this._slotWorld(s.data, this.slot[i], spinC, spinS, scale, _v1);
        this.tgt[i3] = _v1.x;
        this.tgt[i3 + 1] = _v1.y;
        this.tgt[i3 + 2] = _v1.z;
      }
    ...
    // the swarm hum: one positional loop parked at the centroid, louder and
    // slightly higher when the whole fleet is on the move
    const meanSpd = spdSum / N;
    if (!this.hum && this.audio.ready) {
      this.hum = this.audio.play('droneswarm', _v2.set(cxSum / N, cySum / N, czSum / N), {
        gain: 0.0, loop: true, refDistance: 26, send: 0.25,
      });
    }
    if (this.hum) {
      this.hum.setPosition(_v2.set(cxSum / N, cySum / N, czSum / N));
      this.hum.setGain(clamp(0.4 + meanSpd * 0.07, 0.4, 0.95));
